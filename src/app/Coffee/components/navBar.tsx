                ...
                {navList.map((nav, i) => (
                  <div key={i}>
                    ...
                    >
                      <Link
                        href={nav.link}
                        className="block py-2 hover:text-[#BC9A6C] hover:underline"
                      >
                        {nav.name}
                      </Link>
                    </div>
                    {nav.dropdown && activeDropdown === i && (
                      <ul className="pl-4 space-y-2">
                        {nav.dropdown.map((item, j) => (
                          <li key={j} className="hover:text-[#BC9A6C] hover:underline">
        ...
        <div className="hidden lg:flex space-x-6 text-white">
          {navList.map((nav, i) => (
            ...
            >
              <Link
                href={nav.link}
                className={`${inter.className} hover:text-[#BC9A6C] hover:underline`}
              >
                {nav.name}
              </Link>
              {nav.dropdown && activeDropdown === i && (
